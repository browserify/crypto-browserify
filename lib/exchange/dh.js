...
DiffieHellman.prototype.generateKeys = function(encoding) {
	...
	switch(encoding) {
		...
		case 'hex':
			return (new Buffer(this.public_key.toByteArray())).toString('hex');
		case 'binary':
			return (new Buffer(this.public_key.toByteArray())).toString('binary');
		case 'base64':
			return (new Buffer(this.public_key.toByteArray())).toString('base64');
		default:
...
DiffieHellman.prototype.computeSecret = function(other_public_key, input_encoding, output_encoding) {
	...
	switch(input_encoding) {
		...
		case 'binary':
			...
			break;
		case 'base64':
			bob_public = new BigInteger(other_public_key, 64); // Doubt this works but meh :P
			break;
		default:
...
DiffieHellman.prototype.getPrivateKey = function(encoding) {
	...
	if(!this.p /* Fix Me! Dodgy Hax R Us (tm) */) throw new Error('Not initialized');
	if(!this.public_key) throw new Error('No private key - did you forget to generate one?');
...
DiffieHellman.prototype.setPublicKey = function(private_key, encoding) {
	// Technically if we were writing this based on the Node.js source
	// the following line would be correct, however due to them wrapping
	// their bindings, it throws Not a buffer instead.
