...
var DH_GENERATOR_2 = new BigInteger('2', 10);
...
function DiffieHellman(sizeOrKey, encoding) {
  ...
  if('number' === typeof sizeOrKey) {
    // Generate Parameters
    generateParams(this, sizeOrKey, DH_GENERATOR_2);
  }
  else
  {
  ...
  this.g = DH_GENERATOR_2; // Node.js Docs specify 2 as the only supported generator
  this.q = null;
}

function generateParams(dh, prime_len, generator) {
  var t1 = new BigInteger(null)
    , t2 = new BigInteger(null)
    , g
    , ok = -1;

  if(!dh.p) dh.p = new BigInteger(null);
  ...
  generatePrime(dh.p, prime_len, t1, t2);
  dh.g = g;

  return ok;
}

function generatePrime(ret, bits, add, rem) {
  var t = new BigInteger(null)
    , found = 0;

  var checks = (bits >= 1300 ?  2 : 
                bits >=  850 ?  3 : 
                bits >=  650 ?  4 : 
                bits >=  550 ?  5 : 
                bits >=  450 ?  6 : 
  ...
  if(bits <= 2) return 0; // TODO: What to do here? What does Node do?

  while(!found) {
    probableSafePrime(ret, bits, add, rem);
    ...
    found = ret.isProbablePrime(checks);
    found = t.isProbablePrime(checks);
  }

  return found;
}

function probableSafePrime(p, bits, padd, rem)
{
  var i
    , ret = 0
    , qadd = new BigInteger(null)
    , t1
    , q;

  //bits--;

  padd.rShiftTo(1, qadd);
  
  // TODO: Check the following is correct.
  // if (!BN_rand(q,bits,0,1)) goto err;
  q = new BigInteger(rng(bits/8));
